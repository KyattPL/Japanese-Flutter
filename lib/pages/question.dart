...
class _QuestionState extends State<Question> {
  ...
  bool isCorrectAnswer(dynamic questionObj, String shownLang) {
    if (shownLang == 'eng' && questionObj['jp'].toLowerCase() == inputController.text.toLowerCase()) {
      return true;
    } else if (shownLang == 'jp' && questionObj['eng'].toLowerCase().contains(inputController.text.toLowerCase())) {
      return true;
    } else {
      return false;
    }
  ...
  Widget build(BuildContext context) {
    ...
    String whichToShow = arguments['option'] == 1 ? 'eng' : 'jp';
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Japanese-Flutter'),
        centerTitle: true,
